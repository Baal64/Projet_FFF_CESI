	...
	<form method="post" action="./index.php">
		...
		<div id="bloc_equipes" class="flex flex_aic flex_sa flex_col">
			...
			<select id="select_equipe_exterieur" name="select_equipe_exterieur">
				<option>Equipe extérieur</option>
	       <?php
		       	foreach ($equipes as $equipe){
		...
		<div id="bloc_arbitres" class="flex flex_aic flex_sa flex_col">
			...
			<select id="select_arbitre_princiçpal" name="select_arbitre_princiçpal">
				<option>Arbitre principal</option>
				<?php
			...
			<select id="select_arbitre_assistant1" name="select_arbitre_assistant1">
				<option>Arbitre assistant 1</option>
				<?php
	       	foreach ($arbitres as $arbitre){
			...
			<select id="select_arbitre_assistant2"  name="select_arbitre_assistant2">
				<option>Arbitre assistant 2</option>
				<?php
	       	foreach ($arbitres as $arbitre){
		...
		<div id="bloc_localisation" class="flex flex_aic flex_sa flex_col">
			...
			<input type="text" name="localisation_base" id="localisation_base" value="<?php echo $fm->getlieu_match() ?>" />
			<input type="text" name="localisation_substitut" id="localisation_substitut" value="<?php echo $fm->getlieu_substitut() ?>" />
		</div>
		<div id="bloc_date" class="flex flex_aic flex_sa flex_col">
			<div class="label">Date du match</div>
			<input type="text" name="date_match" id="date_match" value="<?php echo deformat_date($fm->getdate_match()) ?>"/>
		</div>
		<button type="submit" name="modification_feuille_match" value="<?php echo $id_match; ?>">Valider</button>
	</form>
	<form method="post" action="index.php">
